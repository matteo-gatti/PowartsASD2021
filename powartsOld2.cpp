    ...
    while(!q.empty()) {
        ...
        for (pair<int, int> adiac : grafo[nodoA].vic)   //per ogni nodo adiacente al nodo in cima alla coda (quello piu vicino a NodoA)
        {
            ...
            if(grafo[nodoB].visited == false){
                ...
                if(nodoA != POWARTS)
                {
                    if ((SPTree[nodoA].distanza > SPTree[nodoB].distanza && SPTree[nodoB].distanza > -1) || (SPTree[nodoA].distanza == -1))
                    {
                        swap(nodoA, nodoB);
                    }
        ...
        if(grafo[nodoBoriginale].visited == false) {q.push(nodoBoriginale); }
       }    
    }
